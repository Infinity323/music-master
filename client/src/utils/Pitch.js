  ...
  Analyzer.prototype = {
    ...
    findTone: function (minFreq, maxFreq) {
      ...
      for (var i = 0; i < this.tones.length; i++) {
        if (
          this.tones[i].db < db - 20.0 ||
          this.tones[i].freq < minFreq ||
          this.tones[i].age < Tone.MIN_AGE
        )
          continue;
        if (this.tones[i].freq > maxFreq) break;

        var score =
          this.tones[i].db - max(180.0, abs(this.tones[i].freq - 300)) / 10.0;

        if (
          this.oldFreq !== 0.0 &&
          abs(this.tones[i].freq / this.oldFreq - 1.0) < 0.05
        )
          score += 10.0;
    ...
    calcTones: function () {
      ...
      for (k = 1, k2 = 2; k <= kMax; k++, k2 += 2) {
        /* complex absolute */
        magnitude = sqrt(
          this.fft[k2] * this.fft[k2] + this.fft[k2 + 1] * this.fft[k2 + 1]
        );
        /* complex arguscosine */
